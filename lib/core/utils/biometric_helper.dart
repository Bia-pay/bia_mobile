...
class BiometricHelper {
  ...
  // /// Helper to get current user ID with fallback to phone
  // static Future<String?> _getCurrentUserId() async {
  //   try {
  //     final authBox = await Hive.openBox('authBox');
  //     final userId = authBox.get('userId');
  ...
  //
  //     // Prefer userId, fallback to phone
  //     final effectiveId = (userId?.toString().isNotEmpty == true)
  //         ? userId.toString()
  //         : (phone?.toString().isNotEmpty == true ? phone.toString() : null);
  //
  //     debugPrint('🔐 Current user ID: $effectiveId (userId=$userId, phone=$phone)');
  //     return effectiveId;
  //   } catch (e) {
  //     debugPrint('❌ Error getting userId: $e');
  //     return null;
  ...
  /// Get saved PIN for current user only
  /// Get saved PIN for current user - checks multiple keys
  // In BiometricHelper.getSavedPin(), change:
